...
public class Itemset {
	...
	private void generateAllNonEmptySubsets(Vector itemsVector, int level,Set allNonEmptySubsets, Itemset currentItemset) {
		...
		// currentItemset, in the second it is
                System.out.println("in generating all nonempty subset");
		boolean itemAdded = false;
		while (true) {
			if (level == itemsVector.size() - 1) {
				// check if it's a proper subset before adding
				if (currentItemset.size() != 0 && currentItemset.size() != itemsVector.size()) {
					allNonEmptySubsets.add(currentItemset);
				}
			} else {
				generateAllNonEmptySubsets(itemsVector, level + 1,allNonEmptySubsets, currentItemset);
			}
			if (itemAdded) {
				break;
			} else {
	...
	public Set generateAllNonEmptySubsets() {
		...
		generateAllNonEmptySubsets(new Vector(items), 0, allNonEmptySubsets,new Itemset());
